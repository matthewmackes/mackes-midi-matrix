...
This repository is in foundation implementation and integration work toward v1.0. The current release line is `0.1.10`. Hardware and network interoperability features are deliberately opt-in until they have the required documentation, fixtures, and validation evidence.
...
The current release is `0.1.10`. Recreate its Fedora/Linux x86_64 bundle with:

The published release is available at
[`v0.1.10`](https://github.com/matthewmackes/mackes-midi-matrix/releases/tag/v0.1.10),
with assets named `mackes-midi-matrix-0.1.10-linux-x86_64.tar.gz` and its `.sha256`
manifest.

```bash
cargo build --workspace --release --locked
bash scripts/package-release.sh 0.1.10
(cd dist && sha256sum -c mackes-midi-matrix-0.1.10-linux-x86_64.tar.gz.sha256)
```
