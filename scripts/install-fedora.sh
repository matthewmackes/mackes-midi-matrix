...
for tool in install getent groupadd useradd systemctl ldconfig; do
...
getent user mackes >/dev/null || useradd --system --home-dir "$state_dir" --shell /sbin/nologin mackes
...
echo "installed; enable/start with: systemctl enable --now mackes-midi-matrix.service"
