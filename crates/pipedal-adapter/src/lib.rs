...
impl Worker {
    ...
    /// Prepares a confirmed, generation-checked forced update check.
    ///
    /// # Errors
    ///
    /// Returns an error when confirmation, generation, or encoding fails.
    pub fn prepare_force_update_check(
        &self,
        generation: u64,
        reply_to: Option<u64>,
        confirmed: bool,
        ...
        if !confirmed {
            return Err("PiPedal update checks require explicit confirmation".into());
        }
        if generation != self.session.generation() {
            return Err("PiPedal update check belongs to an old session generation".into());
        }
        mackes_pipedal_connector::encode_request(&mackes_pipedal_connector::Request::<()> {
            message: "forceUpdateCheck".into(),
            reply_to,
            body: None,
        })
        .map_err(|error| error.to_string())
    ...
    /// Prepares a confirmed, generation-checked port monitor subscription.
    ///
    /// # Errors
    ///
    /// Returns an error when confirmation, generation, payload validation, or encoding fails.
    pub fn prepare_monitor_port(
        &self,
        generation: u64,
        request: mackes_pipedal_connector::MonitorPort,
        reply_to: Option<u64>,
        confirmed: bool,
    ) -> Result<Vec<u8>, String> {
        if !confirmed {
            return Err("PiPedal port monitoring requires explicit confirmation".into());
        }
        if generation != self.session.generation() {
            return Err("PiPedal port monitor belongs to an old session generation".into());
        }
        request.validate()?;
        mackes_pipedal_connector::encode_request(&mackes_pipedal_connector::Request {
            message: "monitorPort".into(),
            reply_to,
            body: Some(request),
        })
        .map_err(|error| error.to_string())
